...
%% Bayes Filter
sensor_pos_x = [4 4 16 16 16 10 10 10 4 10 16 5];
sensor_pos_y = [7 17 3 7 17 3 7 17 12 12 12 4];
...
for i=1:20
    for j=1:20
        if i+2 <= 20 && j+1 <= 20
            state(j+1,i+2,j,i) = 1/8;
        end
        if j+2 <= 20 && i+1 <= 20
            state(j+2,i+1,j,i) = 1/8;
        end
        if i+2 <= 20 && j+2 <= 20
            state(j+2,i+2,j,i) = 1/8;
        end
        if j+1 <= 20
            state(j+1,i,j,i) = 1/8;
        end
...
state = state / sum(sum(sum(sum(state))));

sensor_pos_model = zeros(20);

fig1 = figure;
fig1.Position = [400 50 1500 900];
waitforbuttonpress
subplot(2,3,2)
Sigma = [1 0; 0 1];
x1 = 1:20;
...
for i=1:20
    i_y = max(sensor_pos_y == i);
    for j=1:20
        i_x = max(sensor_pos_x == j);
        if i_y == 1 && i_x == 1
...
while true
    ...
    for i=1:20
        for j=1:20
            if randx == 1 && randy == 1
                if j+1 <= 20
                    tmp(i,j+1) = tmp(i,j+1) + bel(i,j) * 1/8;
                end
                if  i+1 <= 20
                    tmp(i+1,j) = tmp(i+1,j) + bel(i,j) * 1/8;
                end
                tmp(i,j) = tmp(i,j) + bel(i,j) * 1/8;
                if i+2 <= 20
                    tmp(i+2,j) = tmp(i+2,j) + bel(i,j) * 1/8;
                end
                if j+2 <= 20
                    tmp(i,j+2) = tmp(i,j+2) + bel(i,j) * 1/8;
                end
                if i+2 <= 20 && j+2 <= 20
                    tmp(i+2,j+2) = tmp(i+2,j+2) + bel(i,j) * 1/8;
                end
                if i+2 <= 20 && j+1 <= 20
                    tmp(i+2,j+1) = tmp(i+2,j+1) + bel(i,j) * 1/8;
                end
                if j+2 <= 20 && i+1 <= 20
                    tmp(i+1,j+2) = tmp(i+1,j+2) + bel(i,j) * 1/8;
                end
            elseif randx == 1
                if j <= 20
                    tmp(i,j) = tmp(i,j) + bel(i,j) * 1/8;
                ...
                if  i+1 <= 20 && j-1 > 0
                    tmp(i+1,j-1) = tmp(i+1,j-1) + bel(i,j) * 1/8;
                end
                if j-1 > 0
                    tmp(i,j-1) = tmp(i,j-1) + bel(i,j) * 1/8;
                end
